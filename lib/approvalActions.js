...
import { evaluateDealCompliance, evaluateForeignBuyerRisk } from './legalPlaybook.js';
...
export async function executeApprovedAction(approval) {
  ...
  const investor = payload.investorId
    ...
    : null;
  if (!investor) {
    return { success: false, error: 'Close approval requires a matched investorId' };
  }
  const foreignCheck = evaluateForeignBuyerRisk(investor.foreignBuyerProfile || {});
  if (foreignCheck.blocked) {
    return {
      success: false,
      error: `Foreign buyer compliance gate blocked approved close action: ${foreignCheck.blockedReasons.join('; ')}`,
      foreignCompliance: foreignCheck
    };
  }
